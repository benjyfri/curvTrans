        ...
        for src, target, rotation_ab, translation_ab, euler_ab in tqdm(test_loader):
            ...
            if not args.DeepBBS_pp:
                ...
                if not args.no_cuda:
                    ...
                    translation_ab_pred = T[-1]; temp_R = torch.eye(3).unsqueeze(0)
...
def wandb_log(example_cnt, epoch, train_loss, train_r_mse_ab, train_r_rmse_ab, train_r_mae_ab, train_t_mse_ab, train_t_rmse_ab, train_t_mae_ab, log_both = 1,  test_loss = 0, test_r_mse_ab = 0, test_r_rmse_ab = 0, test_r_mae_ab = 0, test_t_mse_ab = 0, test_t_rmse_ab = 0, test_t_mae_ab = 0 ):
    ...
    if log_both:
        wandb.log({"epoch": epoch, "train_loss": train_loss, "train_r_mse_ab": train_r_mse_ab,
               "train_r_rmse_ab": train_r_rmse_ab, "train_r_mae_ab": train_r_mae_ab,
               "train_t_mse_ab": train_t_mse_ab, "train_t_rmse_ab": train_t_rmse_ab,
               "train_t_mae_ab": train_t_mae_ab, "test_loss": test_loss, "test_r_mse_ab": test_r_mse_ab,
                   "test_r_rmse_ab": test_r_rmse_ab, "test_r_mae_ab": test_r_mae_ab,
                   "test_t_mse_ab": test_t_mse_ab, "test_t_rmse_ab": test_t_rmse_ab,
                   "test_t_mae_ab": test_t_mae_ab}, example_cnt)
    else:
        wandb.log({"epoch": epoch, "train_loss": train_loss, "train_r_mse_ab": train_r_mse_ab,
                   "train_r_rmse_ab": train_r_rmse_ab, "train_r_mae_ab": train_r_mae_ab,
                   "train_t_mse_ab": train_t_mse_ab, "train_t_rmse_ab": train_t_rmse_ab,
                   "train_t_mae_ab": train_t_mae_ab}, example_cnt)
...
def main():
    ...
    parser.add_argument('--shape_multiplier',  type=int, default=1, metavar='N')
    parser.add_argument('--normalize_shape',  type=int, default=0, metavar='N')
    ...
    added_dimensions = 0
    args.SVDHEAD_emb_dims = args.emb_dims
    if args.add_before_transformer:
        args.zero_padding = ((args.n_heads) - ((args.emb_dims + added_dimensions) % (args.n_heads))) % (args.n_heads)
        args.emb_dims = (args.emb_dims + added_dimensions) + args.zero_padding
        args.SVDHEAD_emb_dims = args.emb_dims
    elif args.add_after_transformer:
        added_dimensions = 4
        args.SVDHEAD_emb_dims = (args.emb_dims + added_dimensions)

    args.added_dimensions = added_dimensions
    ...
    if args.eval:
        ...
        if args.use_wandb:
            ...
            wandb.init(project="Curvature-transformer-POC-fixedHK",name=args.exp_name, config=args.__dict__)
